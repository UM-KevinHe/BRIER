...
ncol(LD.mat) == nrow(SNP.info)
```

For non-genetic predictors, or when LD blocks are not used, only the reference predictor matrix X is required. In this case, `calLD()` computes the reference covariance/correlation matrix across predictors directly.

```{r, echo = TRUE, eval = FALSE}
LD.list <- calLD(X, tau = 0)
LD.mat <- LD.list[[1]]
keep.idx <- LD.list[[3]]
length(keep.idx) == ncol(X)
```
