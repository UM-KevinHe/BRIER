...
Current tool surface (v1.0.0):
  ...
  * inspect_user_data      - heuristic inspection; supports .csv/.tsv too.
  * list_data_directory    - list .rda / .RData / .rds files in a directory.
...
def inspect_user_data(
    ...
    Supports .rds, .rda, .RData, .csv, .tsv, .txt formats. PLINK and VCF
    are not yet supported (planned for v1.2).
    ...
    return _run_r("inspect_user_data.R", {
...
def list_data_directory(dir_path: str, recursive: bool = False) -> dict:
    """List R data files (.rda / .RData / .rds) in a local directory.
...
def _boundary_optimum_notice(eta_min, eta_list, where: str = "selection"):
    ...
    component of eta_min equals the grid maximum, since a boundary optimum
    in any dimension suggests the ceiling may be too low.
    """
    grid_max = _eta_grid_max(eta_list)
    if grid_max is None or eta_min is None:
        return None
    ...
    try:
        ...
        return None
    at_boundary = any(abs(c - grid_max) < 1e-9 for c in comps)
    if not at_boundary:
        return None
    suggested = grid_max * 5
    return (
        f"Selected eta ({grid_max:g}) is at the top of the {where} grid. "
...
def brier_i(
    ...
    Args:
        ...
        multi_method: One of "stacking" (recommended default), "PCA",
...
def brier_i_cv(
    ...
    Args:
        Same as brier_i, with two extras:
...
def cal_ld(
    ...
    Args:
        data_path: Path to .rda/.RData/.rds with the X matrix.
        X_expr: R expression for the genotype reference panel matrix.
            Shape n_ref x p.
