...
#' @param sumstats A data.frame of marginal summary statistics. Required when
#'   \code{LDB} is supplied; must contain \code{CHR}, \code{BP}, and
#'   \code{corr} columns.
...
#' ld <- calLD(X, sumstats = sumstats, LDB = LDB, tau = 0.05)
#' }
#'
#' @export
calLD <- function(X, sumstats, LDB = NULL, tau = 0) {
  ...
  if (is.null(LDB)) {
    ...
    if (length(nz) != ncol(X)) {
      ...
      center <- center[nz]
      scale <- scale[nz]
      warning(paste("Constant columns are removed:", paste(removed, collapse = ", ")), call. = FALSE)
    }
    ...
    if (ncol(X) != nrow(sumstats)) {
      stop("The dimension of reference X and GWAS summary does not match.", call. = FALSE)
    }
    chr <- sumstats$CHR
    pos <- sumstats$BP
    XtY <- sumstats$corr
    if (is.null(chr) || is.null(pos) || is.null(XtY)) {
      stop("GWAS summary must contain CHR, BP, and corr columns.", call. = FALSE)
    }
